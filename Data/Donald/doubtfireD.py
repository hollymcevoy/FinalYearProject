...
def compare_text_files(file1, file2):
    with codecs.open(file1, "r", encoding='utf-8', errors='ignore') as f1, \
         ...
         lines1 = set(line[:19] for line in f1)
         lines2 = set(line[:19] for line in f2)

    deleted_tweets = lines1 - lines2
    deleted_lines = []
    with codecs.open(file1, "r", encoding='utf-8', errors='ignore') as f:
        for line in f:
            if line[:19] in deleted_tweets:
...
deleted_tweets = compare_text_files("donaldNew10.txt", "donald10.txt")
